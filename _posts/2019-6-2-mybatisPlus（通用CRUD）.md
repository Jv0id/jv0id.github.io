...
```
1)	Integer upmodify_dateById(@Param("et") T entity);

2)	Integer upmodify_dateAllColumnById(@Param("et") T entity)
```
...
```java
	 ...
	 */
	@Test
	public void testCommonUpmodify_date() {
		//初始化修改对象
		Employee employee = new Employee();
		employee.setId(7);
		...
		//Integer result = employeeMapper.upmodify_dateById(employee);
		Integer result = employeeMapper.upmodify_dateAllColumnById(employee);
...
- MappedStatement：一个MappedStatement 对象对应 Mapper 配置文件中的一个 
   select/upmodify_date/insert/delete 节点，主要描述的是一条SQL 语句
