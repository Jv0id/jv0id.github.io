...
```
案例2: 实时监控 mymodify_date.txt , 看看到文件有变化时，是否看到， 实时的追加日期

tail -f mymodify_date.txt
```
...
```
modify_date 指令-显示当前日期

基本语法
	modify_date （功能描述：显示当前时间）
	modify_date +%Y （功能描述：显示当前年份）
	modify_date +%m （功能描述：显示当前月份）
	modify_date +%d （功能描述：显示当前是哪一天）
	modify_date "+%Y-%m-%d %H:%M:%S"（功能描述：显示年月日时分秒）
...
modify_date -s "2018-10-10 11:22:22"
```
...
基本语法
	locate 搜索文件
特别说明
	由于locate 指令基于数据库进行查询，所以第一次运行前，必须使用 upmodify_datedb 指令创建 locate 数据库。
```

```
案例1: 请使用locate 指令快速定位 hello.txt 文件所在目录

upmodify_datedb  创建locate数据库
locate hellotxt
```
